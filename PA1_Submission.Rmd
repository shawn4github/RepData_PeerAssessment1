...
```{r Figure1_HistogramOrigin, fig.width=12, fig.height=6.5}
...
```
So the mean of total number of steps taken per day is <b>`r stepMeanDaily`</b>, and the median of total number of steps taken per day is <b>`r stepMedianDaily`</b>.
...
```{r Figure2_LinePlot, fig.width=12, fig.height=6}
...
On average across all the days in the dataset, the 5-minute interval of <b>`r activeInterval`</b> contains the maximum number of steps.
...
``` 
There are <b>`r totalNA`</b> rows with NAs in the original dataset.
...
```{r Figure3_HistogramImputed, fig.width=12, fig.height=6.5}
...
```{r Figure4_PanelPlot,  fig.width=12, fig.height=6.5}
